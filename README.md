...
The final validation accuracy achieved was approximately 87%, however, the model still has serious defects. In particular, probably the biggest flaw is that fact that it only classifies articles as either positive or negative and not neutral. This means that many articles that contain no sentiment, which are irrelevant for our purposes, will still be classified and returned by the user interface spoken about below. Ideally, the model would be a multi-class classification model with neutral as an additional class so that these articles could be filtered out. Besides from this, the model also has problems when a sentence will have positive sentiment but contains words that it has associated with negative sentiment, i.e., "cost of living decreases as inflation subsides" is classified as negative by the model. This could be improved on by further investigating and implementing other models such as LSTMs and GRUs models along with augmenting and further cleaning the data set combined.csv.
